...
knitr::include_graphics("Figures/Buy_Happiness.jpeg")
```
...
Average <- HappinessReport %>% 
              group_by(Country) %>% 
              #Specify happiness score as column for summary statistics
              summarise_at(vars(Happiness_Score), 
              #Specify summary statistics as computing mean
...
Top10 <- Average %>% 
            #Arrange happiness score in descending order
            arrange(desc(Happiness_Score)) %>% 
            #Select the first 10 data points
            slice(1:10) %>% 
            #Rearranging happiness score so highest score is plotted first
            arrange(Happiness_Score) %>% 
            #Mutate country column to display correct order
            mutate(Country = factor(Country, levels = Country)) 

#Plot top 10 happiest countries
#Add mappings to ggplot
p_top10 <- ggplot(Top10, aes(Country, Happiness_Score, fill = Happiness_Score)) 

p_top10 <- p_top10 +           #Add geom layer to ggplot 
           geom_col() +        #Plot x and y as a bar chart
           coord_flip() +      #Making the bars horizontal
           #Fill bars based on happiness score
           scale_fill_gradient2(low = "#FFF9C4", high = "#FFEB3B", mid = "#FFF176", 
           midpoint = median(Top10$Happiness_Score)) + 
           geom_text(aes(label = round(Happiness_Score, 3)), #Add score on bar
                     nudge_y = -.55, color = "black") + 
           #Add labels
...
Least10 <- Average %>% 
              #Arrange happiness score in ascending order
              arrange(Happiness_Score) %>%  
              #Select the first 10 data points
              slice(1:10) %>% 
              #Rearranging scores so they are plotted from lowest to highest
              arrange(desc(Happiness_Score)) %>% 
              #Mutate country column to display correct order
              mutate(Country = factor(Country, levels = Country)) 
...
#Add mappings to ggplot
p_least10 <- ggplot(Least10, aes(Country, Happiness_Score, fill = Happiness_Score)) 
...
p_least10 <- p_least10 +  #Add geom layer to ggplot
             ...
             geom_text(aes(label = round(Happiness_Score, 3)), 
                       nudge_y = -.55, color = "black") + #Add score on bar
             #Add labels
             labs(title = "Top 10 Least Happy Countries on Average Between 2011-2021",
             subtitle = "0 = Worst Possible Life, 10 = Best Possible Life", 
             y = "Average Happiness Score", 
             caption = "Source: World Happiness Report, Gallup World Poll") + 
...
```{r, show plot, echo=FALSE, fig.show="hold", out.width="55%"}
plot(p_top10)
plot(p_least10)
```
...
Year <- list("2011", "2012", "2013", "2014", "2015", "2016", 
...
for(i in 1:11) {
  ...
  p_out <- p + geom_map(map = World_Map_Joined, aes(map_id = region, 
                        fill = Happiness_Score), linewidth = 0.25) + 
          #Filling countries based on happiness score
           scale_fill_gradient2(low = "#0D47A1", high = "#FFEB3B", mid = "#BBDEFB", 
                                midpoint = 5, name = name, limits = c(2, 8)) + 
...
One of the plots generated by the for loop has been included in this document however, the visualisation has been animated into a GIF which can be seen on my GitHub pages. 
```{r, image2, echo=FALSE}
knitr::include_graphics("Figures/plot_2011.png")
```
...
p_rich <- p_rich + #Adding geom layer to ggplot
          ...
          #Filling bars based on average income
          scale_fill_gradient2(low = "#FFF9C4", high = "#FFEB3B", mid = "#FFF176", 
          ...
          labs(title = "Top 10 Richest Countries based on average income per capita ($)
               in 2021", #Adding labels
          ...
          theme(plot.title = element_text(face = "bold"), #Make title bold
                plot.title.position = "plot", #Change title position
                panel.border = element_blank(), #Remove background
                legend.position="none") #Remove legend
...
p_poor <- p_poor +
          ...
          #Adding income next to bar
          geom_text(aes(label = dollar(Average.Income)), hjust = -0.5, color = "black") + 
          ylim(0, 90000) + #Setting scale to match top 10 rich countries bar chart
          labs(title = "Top 10 Poorest Countries based on average income per capita ($) 
               in 2021", 
          x= "Country", y = "Average Income Per Capita ($)", 
          caption = "Source: United Nations Development Programme") +
          theme_bw() +
...
```{r, show plot2, echo=FALSE, fig.show="hold", out.width="55%"}
plot(p_rich)
plot(p_poor)
```
...
```{r relationship, fig.align='center'}
...
#Creating scatterplot - ggiraph package is used to make scatterplot interactive -
#when mouse hovers over points on the plot, country name, happiness score and 
#average income will be visible
p_both <- ggplot(Joined_Data, aes(Average.Income, Happiness_Score)) +
          ...
          geom_smooth(method="lm", formula= y ~ log(x), se = FALSE, color = "black", 
                      size = 0.5) + #Add logarithmic regression line
          #Add labels
...
plot(p_both)
```
The scatterplot above has been included without the interactive element to support the pdf format. The girafe() function has been used in the html link to create an interactive plot which provides information of each point when the mouse is hovered over it. This version of the scatterplot can be seen on my GitHub pages. 
